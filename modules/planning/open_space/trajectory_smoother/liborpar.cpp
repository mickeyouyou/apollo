...
void portfolio(const int N, const int Nmat, const double delta, const int Nopt,
  ...
  for (n = 0; n < Nmat; n++) v = v / (1.0 + delta * L[n]);
  delete[](B);
  delete[](S);
}
...
#pragma omp parallel firstprivate(ADOLC_OpenMP_Handler)
  {
    ...
    trace_on(init);
    for (k = 0; k < N; k++) La[k] <<= 0.050000;
    for (j = 0; j < Nmat; j++) za[j] <<= z[init][j];
...
#pragma omp barrier
    if (index == 0) {
      ...
      for (l = 0; l < npath; l++) {
        vtot += v[l];
        for (k = 0; k < N; k++) gradtot[k] += grad[l][k];
      }
      vtot = vtot / npath;
      for (k = 0; k < N; k++) gradtot[k] /= npath;
  ...
  std::cout << "Total time on CPU:" << double(duration.count() * 1000)
            << std::endl;
