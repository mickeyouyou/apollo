...
bool InitialCuda() {
  ...
  // TODO(fengzongbao): USE MORE CUDA DEVICE PARAMETERS TO ACCELERATE LIKE
  // STREAM, WRAP SIZE and so on.
...
__global__ void fill_lower_left_gpu(int *iRow, int *jCol, unsigned int *rind_L,
  ...
  // TODO(fengzongbao): use 1D data struct
  if (i < nnz_L) {
    iRow[i] = rind_L[i];
    jCol[i] = cind_L[i];
  }
...
bool fill_lower_left(int *iRow, int *jCol, unsigned int *rind_L,
  ...
  // TODO NO NEED TO USE TYPE dim3 for 1D data
  dim3 block(BLOCK_1);
  ...
  // TODO use pined-memory or unifined memory to accelarate
