...
int main() {
  ...
  for (i = 0; i < n; i++) x[i] = log(1.0 + i);

  /* Tracing of function c(x) */

  trace_on(tag);
  for (i = 0; i < n; i++) xad[i] <<= x[i];

  ceval_ad(xad, cad);

  for (i = 0; i < m; i++) cad[i] >>= c[i];
  trace_off();

  printf("\n c =  ");
  for (j = 0; j < m; j++) printf(" %e ", c[j]);
  printf("\n");
  ...
  auto duration1 = std::chrono::duration<double>(t3 - t2);
  std::cout << "Total time for full Jacobian on CPU:"
            << double(duration1.count() * 1000) << std::endl;
  printmat(" J", m, n, J);
  printf("\n");
  ...
  auto duration2 = std::chrono::duration<double>(t5 - t4);
  std::cout << "Total time for sparse Jacobian on CPU:"
            << double(duration2.count() * 1000) << std::endl;
  ...
  for (i = 0; i < m; i++) {
    printf(" %d: ", i);
    for (j = 1; j <= (int)JP[i][0]; j++) printf(" %d ", JP[i][j]);
    printf("\n");
  }
  printf("\n");
  ...
  for (i = 0; i < m; i++) free(JP[i]);
  free(JP);
  myfree2(J);

  for (i = 0; i < n; i++) delete[] Seed[i];
  delete[] Seed;
