...
__global__ void portfolio_kernel(double* inx, double* outy, double* dery) {
  ...
  for (i = 0; i < N; i++) L[i] = inx[index2 * dim + index * (N + Nmat) + i];
  for (i = 0; i < Nmat; i++)
  ...
  for (i = 0; i < Nopt; i++) {
    ...
    if (-swapval.getValue() > 0) y = y - 100.0 * swapval;
  }

  // apply discount //

  for (n = 0; n < Nmat; n++) y = y / (1.0 + delta * L[n]);
...
int main() {
  ...
  for (int k = 0; k < npath; k++) {
    for (int s = 0; s < N + Nmat; s++) host[k * (N + Nmat) + s] = xp[k][s];
  }
  ...
  for (i = 0; i < npath; i++) {
    for (j = 0; j < N + Nmat; j++) grad[i][j] = hostder[i * (N + Nmat) + j];
  }

  vtot = 0;
  for (i = 0; i < npath; i++) {
    vtot += v[i];
    for (j = 0; j < N; j++) gradtot[j] += grad[i][j];
  }
  vtot = vtot / npath;
  for (j = 0; j < N; j++) gradtot[j] /= npath;

  auto t2 = std::chrono::steady_clock::now();
  auto duration = std::chrono::duration<double>(t2 - t1);

  std::cout << "Total time on CUDA:" << double(duration.count() * 1000)
            << std::endl;
